...
# --- Exit Flag Detection Functions ---
check_exit_flag_support() {
    local validator_path="$1"
    
    if [ ! -x "${validator_path}" ]; then
        return 1  # Can't check, assume flag not supported
    fi
    
    # Check if --no-wait-for-exit exists in exit subcommand help
    local help_output
    help_output=$("${validator_path}" exit --help 2>&1)
    
    if echo "${help_output}" | grep -q -- "--no-wait-for-exit"; then
        return 0  # Flag is supported
    else
        return 1  # Flag not found in help
    fi
}
...
get_validator_version_string() {
    ...
    version_output=$("${validator_path}" --version 2>/dev/null | head -n1)
    echo "${version_output}"
}

regenerate_exit_script() {
    local validator_path="$1"
    local ledger_dir="$2"
    local exit_script_path="$HOME/exit-validator.sh"
    
    echo -e "${CYAN}Regenerating exit-validator.sh convenience script...${NC}"
    
    local version_string
    version_string=$(get_validator_version_string "${validator_path}")
    echo -e "${CYAN}Validator version: ${version_string}${NC}"
    
    local exit_flags="--max-delinquent-stake 10 --min-idle-time 0"
    
    echo -e "${CYAN}Checking if --no-wait-for-exit flag is supported...${NC}"
    if check_exit_flag_support "${validator_path}"; then
        exit_flags="${exit_flags} --no-wait-for-exit"
        echo -e "${GREEN}Flag --no-wait-for-exit detected in help output - will use it${NC}"
    else
        echo -e "${YELLOW}Flag --no-wait-for-exit not found in help output - omitting${NC}"
    fi
    
    cat > "${exit_script_path}" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    echo -e "${GREEN}exit-validator.sh regenerated at ${exit_script_path}${NC}"
}
...
if check_exit_flag_support "${VALIDATOR_EXECUTABLE_PATH_UPGRADE}"; then
    USE_NO_WAIT_FLAG=true
    echo -e "${GREEN}Flag --no-wait-for-exit is supported by this validator version${NC}"
else
    USE_NO_WAIT_FLAG=false
